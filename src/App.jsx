...
// Snapshot a sheet node onto a single landscape page, scaled to fit.
...
export default function SchedulingHub({ session, onSignOut }) {
  ...
  async function exportSchedulePdf() {
    ...
    try {
      const fmt = (d) => `${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
      const range = activeWeek ? `${fmt(activeWeek[0].date)}-to-${fmt(activeWeek[6].date)}` : "week";
      if (scheduleView === "foh" || scheduleView === "bohkitchen") {
        ...
        const node = buildScheduleSheetNode({
          ...
          weekLabel: weekRangeLabel(activeWeek),
          days: sheetDays(activeWeek),
          todayIdx: sheetTodayIdx(activeWeek),
          groups: isFoh
            ? buildFohSheetGroups(activeWeek)
            : [
                { label: "Kitchen", rows: buildGroupRows("kitchen", activeWeek) },
                { label: "BOH", rows: buildGroupRows("boh", activeWeek) },
              ],
          managerOn: buildManagerOn(activeWeek),
        });
